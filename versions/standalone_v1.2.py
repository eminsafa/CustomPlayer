...
class VLCControllerApp:
    ...
    def create_widgets(self):
        title_bar = tk.Frame(self.master, bg='#1c1c1c', height=30)
        title_bar.pack(side=tk.TOP, fill=tk.X)

        # the user will initiate the window move.
        title_bar.bind('<Button-1>', self.click_window)
        title_bar.bind('<B1-Motion>', self.drag_window)

        # --- ADDED CODE: Close button ---
        close_button = tk.Button(title_bar, text='X', command=self.on_closing, bg='red', fg='white', relief='flat', font=("Helvetica", 10, "bold"))
        ...
        self.prev_subtitle_btn = tk.Button(controls_container, text="Previous", command=self.previous_subtitle,
                                           state=tk.DISABLED, bg="#3a3a3a", fg="white", font=("Helvetica", 12))
        self.prev_subtitle_btn.pack(side=tk.LEFT, padx=(5, 0), pady=5)
        self.skip_subtitle_btn = tk.Button(controls_container, text="Next", command=self.skip_subtitle, state=tk.DISABLED,
