...
class SubtitleRepeaterApp:
    def __init__(self, root):
        self.root = root
        self.root.title("Subtitle Repeater Player (V4.2 - Repeating Fixed)")
        ...
        try:
            ...
            messagebox.showerror("VLC Error", f"Could not initialize VLC. Error: {e}")
            self.root.destroy()
            return
    ...
    def load_srt(self, srt_path):
        for enc in ['utf-8', 'latin-1', 'windows-1252']:
            try:
                ...
                continue
        self.subtitles = None
        messagebox.showerror("Error", "Could not load subtitle file.")
    ...
    def perform_smooth_seek(self, time_ms):
        if self.is_system_seeking:
            return
        self.is_system_seeking = True
        self.seek_target_time = time_ms
        self.player.pause()
        self.event_manager.event_attach(vlc.EventType.MediaPlayerTimeChanged, self.on_seek_complete)
        self.player.set_time(time_ms)

    def on_seek_complete(self, event):
        """
        Event handler called by VLC when the time changes. We use it to
        confirm our seek is finished. This version is more robust.
        """
        if not self.is_system_seeking:
            return

        new_time = event.u.new_time

        # Check if the new time is reasonably close to our target.
        # Increased tolerance to 250ms for reliability.
        if abs(new_time - self.seek_target_time) < 250:
            # 1. Immediately set the flag to False to prevent re-entry.
            self.is_system_seeking = False

            # 2. Detach the listener.
            self.event_manager.event_detach(vlc.EventType.MediaPlayerTimeChanged)

            # 3. Resume playback if appropriate.
    ...
    def on_slider_release(self, event):
        if self.player.get_media() and self.is_user_seeking:
            self.is_user_seeking = False  # Set flag early
            pos = float(self.progress_scale.get()) / 1000.0
            target_time_ms = int(self.player.get_length() * pos)
            self.perform_smooth_seek(target_time_ms)
            if self.subtitles:
    ...
    def update_ui(self):
        if self.is_system_seeking or self.is_user_seeking:
        ...
        if length > 0:
            self.progress_scale.set(current_time_ms / length * 1000)
        ...
        if self.in_repeat_loop:
            if current_time_ms >= self.repeat_end_time or current_time_ms < self.repeat_start_time - 50:
                self.repeat_counter += 1
                if self.repeat_counter < int(self.repeat_spinbox.get()):
                    self.perform_smooth_seek(self.repeat_start_time)
                else:
                    self.in_repeat_loop = False
            ...
            if sub.start.ordinal <= current_time_ms < sub.end.ordinal:
                ...
                if abs(current_time_ms - self.repeat_start_time) > 100:
                    self.perform_smooth_seek(self.repeat_start_time)
